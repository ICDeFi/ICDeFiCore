var CartController = function () {
    ...
    function registerEvents() {
        $('body').on('click', '.btn-delete', function (e) {
            ...
            $.ajax({
                ...
                success: function () {
                    be.notify('Removing product is successful.', 'success');
                    loadHeaderCart();
                    loadData();
                }
            });
        ...
        $('body').on('keyup', '.txtQuantity', function (e) {
            ...
            if (q > 0) {
                $.ajax({
                    ...
                    success: function () {
                        be.notify('Update quantity is successful', 'success');
                        loadHeaderCart();
                        loadData();
                    }
                });
            } else {
                be.notify('Your quantity is invalid', 'error');
            }
        ...
        $('#btnClearAll').on('click', function (e) {
            ...
            $.ajax({
                ...
                type: 'post',
                success: function () {
                    be.notify('Clear cart is successful', 'success');
                    loadHeaderCart();
                    loadData();
                }
            });
    ...
    function getColorOptions(selectedId) {
        var colors = "<select class='form-control ddlColorId'> <option vaule='0'></option>";
    ...
    function getSizeOptions(selectedId) {
        var sizes = "<select class='form-control ddlSizeId'> <option vaule='0'></option>";
                    ...
                    render += Mustache.render(template,
                        {
                            ...
                            Amount: be.formatNumber(item.Price * item.Quantity, 0),
                            Url: '/' + item.Product.SeoAlias + "-p." + item.Product.Id + ".html"
                        });
                    totalAmount += item.Price * item.Quantity;
                });
                $('#lblTotalAmount').text(be.formatNumber(totalAmount, 0));
                if (render !== "")
